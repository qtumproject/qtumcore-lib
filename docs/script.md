...
This is the most commonly used transaction output script. It's used to pay to a QTUM address (a QTUM address is a public key hash encoded in base58check)
...
Pay to script hash outputs are scripts that contain the hash of another script, called `redeemScript`. To spend QTUMs sent in a p2sh output, the spending transaction must provide a script matching the script hash and data which makes the script evaluate to true.  This allows to defer revealing the spending conditions to the moment of spending. It also makes it possible for the receiver to set the conditions to spend those QTUMs.
...
`Script` has an easy interface to parse raw scripts from the network or qtumd, and to extract useful information. An illustrative example (for more options check the API reference)
